...
export default function Hero() {
  return (
    <>
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center overflow-hidden bg-dark">
        {/* Dot grid */}
        <div
        ...
        {/* Animated glow orb */}
        <motion.div
          className="absolute pointer-events-none w-[700px] h-[700px] rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
          style={{
            background: "radial-gradient(circle, rgba(255,107,53,0.18) 0%, rgba(255,107,53,0.06) 40%, transparent 70%)",
          }}
          animate={{
            scale: [1, 1.12, 1],
            opacity: [0.7, 1, 0.7],
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 pt-32 pb-24">
          ...
          >
            <span className="text-[11px] font-semibold uppercase tracking-[0.16em] block mb-8 text-primary">
              LINK BUILDING - AEO - GEO
            </span>
          </motion.div>

          <div className="leading-[1.05] tracking-tight text-white mb-6 font-heading font-extrabold text-[clamp(44px,7vw,84px)]">
            <motion.div variants={stagger} initial="hidden" animate="visible" className="flex flex-wrap justify-center gap-x-4 mb-2">
              {words1.map((word, i) => (
                <motion.span key={i} variants={fadeUp}>{word}</motion.span>
              ))}
            </motion.div>
            <motion.div variants={stagger} initial="hidden" animate="visible" className="flex flex-wrap justify-center gap-x-4">
              {words2.map((word, i) => (
                <motion.span key={i} variants={fadeUp}>{word}</motion.span>
              ))}
              <motion.span variants={fadeUp} className="text-primary">AI.</motion.span>
            </motion.div>
          </div>
          ...
          <motion.p
            ...
            className="text-lg md:text-xl max-w-2xl mx-auto mt-6 leading-relaxed text-white/65"
          >
          ...
          <motion.div
            ...
            className="mt-12 pt-8 flex flex-wrap items-center justify-center gap-6 border-t border-white/10"
          >
            {["White-Hat Only", "AEO + GEO Ready", "Global Service"].map((item) => (
              <span key={item} className="text-sm flex items-center gap-2 text-white/50">
